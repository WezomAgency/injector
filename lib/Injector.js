...
class Injector {
	 ...
	 * @return {Injector}
	 */
	svgSpritemap (sources, filename, spriteOptions) {
		this.__defaults.svgSpritemap.push({ sources, filename, spriteOptions });
		return this;
	}
	...
	get webpackOptionPlugins () {
		...
		this.__defaults.svgSpritemap.forEach(svgSpritemap => {
			let svg = path.dirname(svgSpritemap.sources);
			let js = path.dirname(this.__defaults.app.distFile);
			let filename = normalizePath(path.join(path.relative(js, svg), svgSpritemap.filename + '.svg'));
			plugins.push(new svgSpritemapPlugin(svgSpritemap.sources, {
				output: {
					filename,
					chunk: {
						name: 'bundled-' + svgSpritemap.filename
					}
				},
				sprite: svgSpritemap.sprite
			}));
		});
