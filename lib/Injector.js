...
class Injector {
	...
	get webpackOptionPlugins () {
		let plugins = this.__defaults.plugins;
		...
		if (this.isWatching && this.__defaults.browserSyncOptions !== null) {
			const _autoReload = !!this.__defaults.browserSyncOptions._autoReload;
			delete this.__defaults.browserSyncOptions._autoReload;
			if (this.isHot) {
				const self = this;
				plugins.push(
					new WriteFilePlugin({
					...
					new BrowserSyncHotPlugin({
						...
						hotMiddleware: {},
						devMiddleware: {
							publicPath: self.__defaults.publicPath,
							noInfo: false,
							quiet: false,
							stats: {
								colors: true,
